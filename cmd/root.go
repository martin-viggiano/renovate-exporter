...
func init() {
	...
	rootCmd.Flags().String("address", ":9090", "Metrics server address")
...
func run(cmd *cobra.Command, args []string) error {
	...
	// Parameters
	watchDir := viper.GetString("path")
	metricsAddress := viper.GetString("address")
	...
	matcher, err := analyzer.NewEngine(reg, matchers.DefaultMatchers())
	if err != nil {
		return err
	}
